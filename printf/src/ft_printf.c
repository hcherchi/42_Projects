...
/*   Updated: 2017/02/15 14:22:29 by hcherchi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*cut_string(t_format *format, char *buf)
{
	...
	if (format->type == 's' && format->accur != -1
		&& format->accur < ft_strlen(buf))
	{
...
char	*ajust_buffer(t_format *format, char *buf)
{
	...
	finalbuf = (format->moins) ?
		ft_strcat(buf, spaces) : ft_strcat(spaces, buf);
	free(spaces);
	return (finalbuf);
}
