...
char *ll_convert(t_format *format, va_list ap)
{
  ...
  if (format->type == 'd' || format->type == 'i')
    converted_arg = itoa_10(va_arg(ap, long long int), format);
  else if (format->type == 'o')
    converted_arg = uitoa_base(va_arg(ap, unsigned long long int), 8, 0, format);
  else if (format->type == 'u')
    converted_arg = uitoa_base(va_arg(ap, unsigned long long int), 10, 0, format);
  else if (format->type == 'x')
    converted_arg = uitoa_base(va_arg(ap, unsigned long long int), 16, 0, format);
  else if (format->type == 'X')
