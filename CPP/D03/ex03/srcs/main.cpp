...
ClapTrap *getPlayer(int index)
{
  ...
  while (_type != "FragTrap" && _type != "NinjaTrap" && _type != "ScavTrap" && !std::cin.eof())
  {
    std::cout << "Class -> ";
  ...
  else if (_type == "NinjaTrap")
    player = new NinjaTrap(_name);
...
int main(void)
{
  ...
  while (player1->getHP() != 0 && player2->getHP() != 0 && !std::cin.eof())
  {
    ...
    {
      if (currentPlayer->getType() == "NinjaTrap")
      {
        if (otherPlayer->getType() == "ScavTrap")
          otherPlayer->takeDamage(((NinjaTrap*)currentPlayer)->ninjaShoebox(*(ScavTrap*)otherPlayer));
        else if (otherPlayer->getType() == "FragTrap")
          otherPlayer->takeDamage(((NinjaTrap*)currentPlayer)->ninjaShoebox(*(FragTrap*)otherPlayer));
        else if (otherPlayer->getType() == "NinjaTrap")
          otherPlayer->takeDamage(((NinjaTrap*)currentPlayer)->ninjaShoebox(*(NinjaTrap*)otherPlayer));
        else if (otherPlayer->getType() == "ClapTrap")
          otherPlayer->takeDamage(((NinjaTrap*)currentPlayer)->ninjaShoebox(*(ClapTrap*)otherPlayer));
      }
      else if (currentPlayer->getType() == "ScavTrap")
